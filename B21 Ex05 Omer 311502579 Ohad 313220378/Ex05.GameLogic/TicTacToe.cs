    ...
    public class TicTacToe
    {
        ...
        private void handleGameOver(ref GameLogic.eGameState io_CurrentState, ref bool io_GameOver)
        {
            if (!io_CurrentState.Equals(GameLogic.eGameState.Draw))
            {
                ...
                //UI.RoundWon(m_CurrentPlayer == m_Player1 ? m_Player2 : m_Player1);
            }
            else
            {
                //UI.Draw();
            ...
            //Player winner = GameLogic.GetWinner(m_Player1.Score, m_Player2.Score);
        ...
        private void getMoveFromPlayer(ref GameLogic.eGameState io_CurrentState, ref bool io_GameOver)
        {
            ...
            bool gameContinues = true; // UI.ChooseNextMove(ref currentPlay);
            ...
            else
            {
                if (!GameLogic.MakeMove(
                    currentPlay,
                    m_Board,
                    m_CurrentPlayer == m_Player1 ? m_Player1.Symbol : m_Player2.Symbol))
                {
                    //UI.InvalidPlay();
                    Thread.Sleep(2000);
                    io_CurrentState = GameLogic.eGameState.ReTry;
